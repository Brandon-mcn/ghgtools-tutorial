...
Most of the functionality in ghgtools comes from the [data.table](https://cran.r-project.org/web/packages/data.table/vignettes/datatable-intro.html) package. This package provides powerful functionality for manipulating date tables.

```{r, include=TRUE, message=FALSE, warning=FALSE}
library(data.table)
```

In order to consolidate emissions data across multiple greenhouse gasses, we need to select global warming potentials (GWPs). You can learn more about different values of global warming potentials by follwoing [this link](https://www.epa.gov/ghgemissions/understanding-global-warming-potentials). ghgtools defaults to the UNFCCC guidelines, which require the use of GWP values from the IPCC's Fifth Assessment Report (AR5).
...
You can browse the emission factor library below:

```{r, echo=FALSE, warning=FALSE, message=FALSE}
EFL_CO2e.narm <- EFL_CO2e
EFL_CO2e.narm[is.na(EFL_CO2e.narm)] <- ""
kbl(EFL_CO2e.narm) %>%
...
<br>
At this point, we have defined each of the required input variables - assets, activities, and emission factors - and loaded the data in the required templates. The following code combines these three datasheets, assignes emission factors, and calculates GHG emissions for each record of activity data. 
